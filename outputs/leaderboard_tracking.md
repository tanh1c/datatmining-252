...
| 0.72184 | `outputs/submissions/next_qwen_5anchor_safest_with_qwen_submission.csv` | Step 15 — 5-anchor stack with Qwen LoRA `qwen 0.20 / e5 0.40 / scincl 0.25 / specter 0.15 / ridge 0.00` | 0.6619 | **Regression** vs `safest_e5` (−0.00210). Qwen single OOF 0.6326, r 0.88 vs encoders (passed both L11/L12 floors); blend probe predicted +0.0026 OOF lift but public regressed. Pattern: OOF up + test_L1 up (0.142→0.152) → public down. **OOF→public ratio went NEGATIVE for the first time.** Suspect: peft adapter chaining bug across folds 2-5 inflated OOF; OR Qwen verbalizer score distribution slightly mis-aligned with public split (label 2 under-predicted by 10 rows). See L17 in lessons_learned.md. |
