...
| **0.72394** | `outputs/submissions/next_e5_4anchor_safest_e5_submission.csv` | **Step 13 — E5 4-anchor stack `e5 0.50 / scincl 0.30 / specter 0.20 / ridge 0.00`, distribution-constrained thresholds** | **0.6593** | Former best — +0.00020 over BGE safest (0.72374). E5-large-v2 OOF 0.6417 (highest single-anchor) but r 0.944 vs BGE = same-family redundancy (L7 boundary). Lift was tiny because E5 = "better BGE" not "new anchor". Picked by L7/L9: lowest test_L1 0.1422 in any sweep (vs anchor 0.156). Step 13 confirmed E5+BGE 5-anchor stacks always underperform 4-anchor with E5 replacing BGE. See L15 in lessons_learned.md. |
| **0.72808** | `outputs/submissions/next_step18b_targeted_blend_ridge_a10_w0p08_submission.csv` | **Step 18b — targeted OOF-error feature calibration on E5 stack: `0.92 * safest_e5 + 0.08 * Ridge(alpha=10) pattern calibrator`** | **0.6602** | **NEW BEST — +0.00414 over E5 safest (0.72394).** Step 18 audit identified systematic false-high keyword traps and false-low KR/CAV/LICS cases; Step 18b used a small regularized calibrator over base score + private-safe pattern/metadata features, not public row correction. Stayed inside L17 safe cap (`test_L1=0.1456 <= 0.147`) and transferred unusually well despite tiny OOF lift. See L20. |
| 0.72184 | `outputs/submissions/next_qwen_5anchor_safest_with_qwen_submission.csv` | Step 15 — 5-anchor stack with Qwen LoRA `qwen 0.20 / e5 0.40 / scincl 0.25 / specter 0.15 / ridge 0.00` | 0.6619 | **Regression** vs `safest_e5` (−0.00210). Qwen single OOF 0.6326 before PEFT fix, r 0.88 vs encoders; public regressed despite OOF lift because test_L1 rose 0.142→0.152. Post-fix clean Qwen single dropped to 0.6205, confirming adapter chaining inflated signal. Clean blend sweep still had no Qwen-bearing candidate inside L17 safe band: best safe remains E5-only; lowest-L1 Qwen blend has L1 0.1523. Do not submit more Qwen 7B candidates. See L17. |
| 0.71453 | `outputs/submissions/next_qwen14b_heavy_5anchor_safe_high_oof_submission.csv` | Step 17c — Qwen14B heavy 5-anchor stack `qwen14b 0.20 / e5 0.40 / scincl 0.25 / specter 0.15 / ridge 0.00` | 0.6681 | **Regression** vs `safest_e5` (−0.00941) despite large OOF lift and safe `test_L1=0.1456`. Qwen14B old-notebook run had PEFT adapter-chaining warnings; local OOF likely inflated/miscalibrated. See L19. |
| 0.71408 | `outputs/submissions/next_qwen14b_heavy_5anchor_risky_high_qwen_submission.csv` | Step 17c — risky high-Qwen stack `qwen14b 0.35 / e5 0.26 / scincl 0.195 / specter 0.13 / ridge 0.065` | 0.6760 | **Regression** vs `safest_e5` (−0.00986). Highest local OOF in the Qwen14B sweep, but `test_L1=0.1718` and public confirmed the distribution/adapter-chain risk. Do not submit more Qwen14B old-notebook stacks. See L19. |
...
| not submitted | `outputs/scholarly_graph_anchor/` | Step 16 scholarly graph/concept metadata anchor (OpenAlex/S2/Crossref/OpenCitations caches, best ExtraTrees, 5x3) | 0.363319 | **Dropped, never submitted.** Diversity vs E5 stack was real (r=0.507), but standalone signal was far too weak. Best safe blend `graph_w0.05` had OOF 0.6591 vs E5 0.6593 and test_L1 0.1456; no graph blend both beat E5 and stayed inside L17 safe band. See L18. |
...
| 0.72403 | `outputs/submissions/next_step22_qwen14b_clean_main_high_oof_safe_l1_submission.csv` | Step 22 clean Qwen14B H200 stack `qwen14b 0.15 / e5 0.3825 / scincl 0.2975 / specter 0.17 / ridge 0` | 0.668669 | **Regression vs Step18b best 0.72808 despite clean fresh-base-per-fold Qwen14B and large local lift (+0.00849).** Test L1 was inside the safe band (0.14555), but changed-row profile still overfit public. See L21. |
| 0.72106 | `outputs/submissions/next_step22_qwen14b_clean_safer_hedge_submission.csv` | Step 22 clean Qwen14B H200 hedge `qwen14b 0.10 / e5 0.495 / scincl 0.225 / specter 0.18 / ridge 0` | 0.667131 | **Worse than main Step22 and Step18b.** Lower Qwen weight and fewer risky changes did not transfer. Clean Qwen14B is useful as a diagnostic/diverse anchor, but current stack weight sweeps should not be promoted. See L21. |
| **0.72808** | `outputs/submissions/next_step23_best_safe_gate_submission.csv` | Step 23 Qwen14B disagreement gate over Step18b; fixed Step18b thresholds, only 12 row-level changes | 0.663062 | **Tied Step18b, did not beat it.** Local gate looked ideal (OOF +0.002886, 12/12 changed rows improved, 0 worsened, `test_L1=0.145552` unchanged), and public did not regress. But it also did not improve over Step18b, so keep only as a private hedge. See L22. |
| **0.73054** | `outputs/submissions/next_step25_bge_m3_best_safe_submission.csv` | **Step 25 — BGE-M3 frozen diagnostic anchor over Step18b: `step18b_bge_m3_ridge_a30p0_w0p02`, low-weight `w=0.02` blend** | **0.661261** | Former best — +0.00246 over Step18b. Small frozen BGE-M3 correction stayed inside the safe band (`test_L1=0.145552`) and changed only 9 test rows. This validated low-weight distribution-safe encoder diagnostics after broad Qwen/Stella/v4-clean sweeps failed. See L23. |
| 0.72820 | `outputs/submissions/next_step35d_best_safe_submission.csv` | Step 35D safe demotion gate over Step25; one CAV/formal synthesis row demoted `2 -> 1` | 0.661699 | **Regression** vs Step25. The single changed public-visible direction was harmful; do not demote CAV/formal synthesis row 2022 to label 1. |
| 0.72602 | `outputs/submissions/next_step36_risky_high_oof_ridge_a3_w0p030_l4_submission.csv` | Step 36 risky BGE-M3 fine sweep: `ridge_a3p0`, `w=0.0300`, `threshold_lambda=4.0` | 0.663105 | **Regression** despite strong OOF and safe-looking `test_L1=0.145552`; changed 9 rows vs Step25. Higher OOF alone did not transfer. |
| **0.73213** | `outputs/submissions/next_step36_risky_small_diff_ridge_a3_w0p0275_l4_submission.csv` | **Step 36 — BGE-M3 fine sweep over Step18b: `ridge_a3p0`, `w=0.0275`, `threshold_lambda=4.0`** | **0.662098** | **NEW BEST — +0.00159 over Step25.** Small-diff probe changed only 5 rows vs Step25 and beat both higher-OOF Step36 probes. `test_L1=0.148908` slightly exceeded the old strict cap, so this was correctly treated as an explicitly risky public probe. See L24. |
| 0.73201 | `outputs/submissions/next_step36_risky_high_oof_ridge_a3_w0p0375_l4_submission.csv` | Step 36 high-OOF BGE-M3 fine sweep: `ridge_a3p0`, `w=0.0375`, `threshold_lambda=4.0` | 0.663312 | Strong result but just below the small-diff winner. Highest OOF in the Step36 probe batch did not win public; changed 11 rows vs Step25. |
| 0.72879 | `outputs/submissions/next_step37_close_oof_micro_submission.csv` | Step 37 micro-sweep around Step36: `ridge_a3p0`, `w=0.02800`, `threshold_lambda=6.0`; only 1 row diff vs Step36 | 0.662340 | **Regression** vs Step36 despite being only 1 changed row away and having slight OOF lift vs Step36. The Step36 changed-row set is extremely brittle; even one extra/moved row can hurt public. |
| 0.72916 | `outputs/submissions/next_step37_best_micro_submission.csv` | Step 37 high-OOF micro-sweep: `ridge_a3p0`, `w=0.02900`, `threshold_lambda=6.0`; 5 rows diff vs Step36 | 0.663513 | **Regression** vs Step36 despite the best local OOF in the BGE-M3 micro-sweep. Confirms not to chase OOF or tiny nearby retunes around the Step36 winner without changed-row evidence. |
| 0.72635 | `outputs/submissions/next_step38_best_lower_oof_submission.csv` | Step 38 lower-weight fine sweep: `ridge_a3p0`, `w=0.02650`, `threshold_lambda=4.0`; 5 rows diff vs Step36 | 0.662763 | **Regression** vs Step36 despite positive OOF lift over Step36 and one fewer diff vs Step25 than the winner. Lowering BGE-M3 weight did not fix the brittle changed-row direction. |
| 0.72879 | `outputs/submissions/next_step38_closest_lower_safe_submission.csv` | Step 38 closest lower-weight probe: `ridge_a3p0`, `w=0.02600`, `threshold_lambda=6.0`; only 1 row diff vs Step36 | 0.662213 | **Regression** vs Step36 and tied the Step37 one-row regression score. A single changed row away from Step36 can still lose ~0.0033 public QWK. |
| **0.73215** | `outputs/submissions/next_step39_best_backbone_oof_submission.csv` | **Step 39 — Step18b backbone retune: internal targeted calibrator `w=0.10` plus fixed BGE-M3 `ridge_a3p0`, `w=0.0275`, `threshold_lambda=4.0`** | **0.662586** | **NEW BEST — +0.00002 over Step36.** Increasing the Step18b targeted calibrator from `0.08` to `0.10` inside the 97.25% backbone gave a tiny public lift despite `test_L1=0.152263` and 5 rows changed vs Step36. See L25. |
| 0.73079 | `outputs/submissions/next_step40_closest_fine_probe_submission.csv` | Step 40 — fine calibrator probe `backbone_weight=0.0975`, fixed BGE-M3 `w=0.0275`, `threshold_lambda=4.0` | 0.662409 | Regression vs Step39 despite only 2 rows changed vs Step39; stop fine calibrator sweeps around the Step39 knife edge. See L26. |
| 0.72062 | `outputs/submissions/next_step41_high_e5_scincl_ridge0_submission.csv` | Step 41 forced large-backbone retune: `E5=0.55 / SciNCL=0.35 / SPECTER=0.10 / Ridge=0`, plus Step39 calibrator+BGE wrapper | 0.659013 | Severe regression vs Step39. Increasing E5/SciNCL while reducing SPECTER broke the public-safe Step13 backbone. See L26. |
| 0.71643 | `outputs/submissions/next_step41_higher_e5_ridge0_submission.csv` | Step 41 forced large-backbone retune: `E5=0.60 / SciNCL=0.30 / SPECTER=0.10 / Ridge=0`, plus Step39 calibrator+BGE wrapper | 0.659128 | Severe regression; more E5 made public worse despite similar local OOF to the 55/35/10 probe. See L26. |
| 0.71719 | `outputs/submissions/next_step41_balanced_high_e5_ridge0_submission.csv` | Step 41 forced large-backbone retune: `E5=0.55 / SciNCL=0.30 / SPECTER=0.15 / Ridge=0`, plus Step39 calibrator+BGE wrapper | 0.659008 | Severe regression and high `test_L1=0.182465`; reducing SPECTER below the Step39 base mix is unsafe. See L26. |
| 0.73022 | `outputs/submissions/next_step41_specter25_from_e5_ridge0_submission.csv` | Step 41 SPECTER-up probe: `E5=0.45 / SciNCL=0.30 / SPECTER=0.25 / Ridge=0`, plus Step39 calibrator+BGE wrapper | 0.659475 | Regression vs Step39; simply increasing SPECTER by taking weight from E5 did not transfer. See L27. |
| **0.73290** | `outputs/submissions/next_step41_specter30_from_scincl_ridge0_submission.csv` | **Step 41 — SPECTER-up backbone retune: `E5=0.50 / SciNCL=0.20 / SPECTER=0.30 / Ridge=0`, plus Step39 calibrator+BGE wrapper** | **0.663141** | **NEW BEST — +0.00075 over Step39.** User hypothesis was right: the failed high-E5 probes reduced SPECTER too much; increasing SPECTER to 30% while taking weight from SciNCL improved both OOF and public. See L27. |
