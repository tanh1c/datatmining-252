...
OUT = ROOT / "outputs" / "step35c_rubric_threshold_micro_gate"
...
def build_split(raw: pd.DataFrame, split: str, abstracts: pd.DataFrame) -> pd.DataFrame:
    ...
    df["nearest_threshold"] = THRESHOLDS[np.argmin(np.abs(score[:, None] - THRESHOLDS[None, :]), axis=1)]
...
def expand_pairs(df: pd.DataFrame, labels: np.ndarray | None = None) -> tuple[pd.Series, np.ndarray | None]:
    ...
    for row_pos, (_, row) in enumerate(df.iterrows()):
        ...
        for cls in [1, 2, 3, 4, 5]:
            texts.append(f"rubric class {cls}: {RUBRICS[cls]} pattern hints: {pattern_hint} paper: {row['paper_text']}")
...
def proposal_frame(df: pd.DataFrame, probs: np.ndarray) -> pd.DataFrame:
    ...
    out["direction"] = np.sign(out["rubric_pred"].astype(int) - out["step25_pred"].astype(int)).astype(int)
    out["proposed_pred"] = out["step25_pred"].astype(int) + out["direction"]
    out["proposed_pred"] = out["proposed_pred"].clip(1, 5).astype(int)
    out["core_context"] = out["core_asp"].eq(1) | out["logic_programming"].eq(1)
    out["risky_context"] = (out["formal_adjacent"].eq(1) | out["ml_adjacent"].eq(1)) & ~out["core_context"]
    return out


def select_mask(df: pd.DataFrame, margin_cap: float, conf_gap: float, max_rows: int, allow_promote_risky: bool, allow_demote_core: bool) -> pd.Series:
    mask = (
        df["direction"].ne(0)
        & df["threshold_margin"].le(margin_cap)
        & df["rubric_gap"].ge(conf_gap)
        & df["proposed_pred"].between(1, 5)
    )
    if not allow_promote_risky:
        mask &= ~((df["direction"] > 0) & df["risky_context"])
    if not allow_demote_core:
        mask &= ~((df["direction"] < 0) & df["core_context"])
    idx = df.index[mask].tolist()
    if len(idx) <= max_rows:
        return mask
    sub = df.loc[idx].copy()
    sub["priority"] = sub["rubric_gap"] - sub["threshold_margin"] + 0.02 * (sub["rubric_conf"])
...
def apply_rule(df: pd.DataFrame, rule: dict[str, Any]) -> np.ndarray:
    ...
    mask = select_mask(df, float(rule["margin_cap"]), float(rule["conf_gap"]), int(rule["max_rows"]), bool(rule["allow_promote_risky"]), bool(rule["allow_demote_core"])).to_numpy(bool)
...
def change_summary(y: np.ndarray, base_pred: np.ndarray, new_pred: np.ndarray, pub_base: np.ndarray, pub_new: np.ndarray, pri_base: np.ndarray, pri_new: np.ndarray, train: pd.DataFrame) -> dict[str, Any]:
    ...
    if changed.any():
        ...
        risky_promotions = int(((new_pred[changed] > base_pred[changed]) & sub["risky_context"].to_numpy(bool)).sum())
        core_demotions = int(((new_pred[changed] < base_pred[changed]) & sub["core_context"].to_numpy(bool)).sum())
    else:
...
def write_pick(label: str, row: pd.Series, train: pd.DataFrame, public: pd.DataFrame, private: pd.DataFrame, sample: pd.DataFrame, y: np.ndarray) -> dict[str, Any]:
    rule = {key: row[key] for key in ["c", "margin_cap", "conf_gap", "max_rows", "allow_promote_risky", "allow_demote_core"]}
    out_dir = OUT / label
    ...
    changed_public = public.loc[pub_pred != public["step25_pred"].astype(int).to_numpy(), ["id", "title", "venue", "step25_pred", "rubric_pred", "rubric_conf", "rubric_gap", "threshold_margin", "proposed_pred"]]
    changed_private = private.loc[pri_pred != private["step25_pred"].astype(int).to_numpy(), ["id", "title", "venue", "step25_pred", "rubric_pred", "rubric_conf", "rubric_gap", "threshold_margin", "proposed_pred"]]
    changed_public.assign(split="public").to_csv(out_dir / "changed_public.csv", index=False)
    changed_private.assign(split="private").to_csv(out_dir / "changed_private.csv", index=False)
    ...
    path = SUBMIT_DIR / f"next_step35c_{label}_submission.csv"
...
def main() -> None:
        ...
        for margin_cap in MARGIN_CAPS:
            for conf_gap in CONF_GAPS:
                for max_rows in MAX_ROWS:
                    for allow_promote_risky in [False, True]:
                        for allow_demote_core in [False, True]:
                            rule = {"c": c, "margin_cap": margin_cap, "conf_gap": conf_gap, "max_rows": max_rows, "allow_promote_risky": allow_promote_risky, "allow_demote_core": allow_demote_core}
    ...
    safe = candidates[
        ...
        & (candidates["risky_promotions"] <= 1)
        & (candidates["core_demotions"] <= 1)
    ...
    show_cols = ["c", "margin_cap", "conf_gap", "max_rows", "allow_promote_risky", "allow_demote_core", "oof_qwk", "oof_lift_vs_step25", "test_l1", "changed", "improved", "worsened", "public_changed", "private_changed", "risky_promotions", "core_demotions"]
    lines = ["# Step35C rubric threshold micro-gate", ""]
    lines.append("This step uses the Step35B rubric-conditioned reranker only as a near-threshold micro-gate over Step25 labels. It does not blend all scores; it applies one generalized confidence rule uniformly to train/public/private.")
    ...
    print("=== Step35C rubric threshold micro-gate ===")
